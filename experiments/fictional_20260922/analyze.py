"""Offline, auditable snapshots of the original-fiction BEFORE/AFTER study.
...
SPEC = importlib.util.spec_from_file_location("fictional_extraction_metrics", SOURCE)
...
MODELS = ("gpt-6-luna", "gpt-6-sol", "gpt-6-astra", "claude-opus-5-5")
SAMPLE_HOPS = (1, 2, 3, 5, 10, 20, 30, 36, 37, 38, 39, 40)
SHUFFLE_SEED = 26092241
STUDY_BUDGET = 10.0
...
def identity_map():
    ...
    return {f"C{index:02d}": {"operator": operator, "model": model}
...
def build(root):
    ...
    for operator in OPERATORS:
        ...
        require(set(m["id"] for m in config["models"]) == set(MODELS) and len(config["models"]) == 4,
    ...
    require(usage + reserve <= STUDY_BUDGET + 1e-6, "Usage plus retained reserves exceeds the new study's $10 budget.")
    ...
    metrics = {
        ...
        "scope": "Only the new original-fiction BEFORE/AFTER study. Prior-study costs and outputs are excluded from its totals.",
        "definitions": {
        ...
        "identity_masking": {"shuffle_seed": SHUFFLE_SEED, "aliases": "C01 through C08",
        ...
        "source_code_hashes": {"analyzer": AUDIT.sha(Path(__file__).read_text(encoding="utf-8")),
                               "audit": AUDIT.sha(DERIVED.AUDIT_PATH.read_text(encoding="utf-8")),
    ...
    metrics["literal_wording_comparison"] = literal_comparison(root, metrics)
    return metrics, mapping, captured


def comparison_row(stream):
    return {"attempts": stream["attempts"], "completed_hops": stream["completed_hops"], "status": stream["status"],
            "provider_refusal_hops": stream["provider_refusal_hops"],
            "unextractable_completed_outputs": len(stream["unextractable_completed_hops"]),
            "first_full_exact_repeat_hop": stream["full_visible_exact_recurrence"]["first_repeat_hop"],
            "first_extracted_exact_repeat_hop": stream["extracted_text_exact_recurrence"]["first_repeat_hop"],
            "final_completed_extracted_words": stream["final_completed_extracted_words"],
            "final_completed_extracted_characters": stream["final_completed_extracted_characters"]}


def literal_comparison(root, metrics):
    old_path = root.parent / "operators_20260922" / "analysis" / "metrics.json"
    comparison = {"scope": "Descriptive comparison only: n=1 trajectory per wording/model/operator. No significance or causal claims.",
                  "matching": "Cells matched by operator label and requested model ID; runs occurred at different times.",
                  "old_source": str(old_path), "new_all_arms_finalized": metrics["all_arms_finalized"],
                  "cost_accounting": "Old literal-run costs never enter the new $10 study totals.", "rows": {}}
    if not old_path.is_file():
        comparison.update(available=False, note="Prior literal metrics were not found at the specified local path.")
        return comparison
    try:
        raw = old_path.read_text(encoding="utf-8")
        old = json.loads(raw)
        for key, stream in metrics["streams"].items():
            comparison["rows"][key] = {"literal": comparison_row(old["streams"][key]),
                                       "original_fiction": comparison_row(stream)}
    except (OSError, ValueError, KeyError) as exc:
        comparison.update(available=False, note="Prior metrics could not be compared: " + type(exc).__name__, rows={})
        return comparison
    comparison.update(available=True, old_source_sha256=AUDIT.sha(raw),
                      old_generated_at=old["generated_at"], old_all_arms_finalized=old["all_arms_finalized"],
                      old_rules={operator: old["arms"][operator].get("rule") for operator in OPERATORS},
                      new_rules={operator: metrics["arms"][operator].get("rule") for operator in OPERATORS})
    return comparison


def literal_comparison_table(comparison):
    lines = ["# Literal wording and explicit original fiction", "", comparison["scope"], "", comparison["matching"], "",
             "The new runs are " + ("finalized." if comparison["new_all_arms_finalized"] else
                                     "an incomplete snapshot; differences below must not be treated as final outcomes."), ""]
    if not comparison["available"]:
        return "\n".join(lines + [comparison["note"], ""])
    lines.extend(["Each paired cell below shows literal / original-fiction values. A null extracted length means the actual final completed "
                  "output was unextractable or no hop completed; no earlier passage is substituted. Unextractable does not mean no fiction.", "",
                  "| Operator/model | Completed hops | Terminal/current status | Provider refusals | Unextractable completed | First full exact repeat | Final extracted words | Final extracted characters |",
                  "| --- | --- | --- | --- | --- | --- | --- | --- |"])
    for key, pair in comparison["rows"].items():
        old, new = pair["literal"], pair["original_fiction"]
        def values(field):
            return cell(old[field]) + " / " + cell(new[field])
        lines.append(f"| {key} | {values('completed_hops')} | {values('status')} | "
                     f"{len(old['provider_refusal_hops'])} / {len(new['provider_refusal_hops'])} | "
                     f"{values('unextractable_completed_outputs')} | {values('first_full_exact_repeat_hop')} | "
                     f"{values('final_completed_extracted_words')} | {values('final_completed_extracted_characters')} |")
    lines.extend(["", comparison["cost_accounting"], "", "Source provenance, exact rules, and paired metrics are recorded in "
                  "`metrics.json` under `literal_wording_comparison`.", ""])
    return "\n".join(lines)
...
def summary_table(metrics):
    ...
    lines = ["# Original-fiction study snapshot", "", "Snapshot: " + metrics["generated_at"] + ". "
             + ("Both arms have final receipts." if metrics["all_arms_finalized"] else
...
def save(root, metrics, mapping, captured):
    ...
    files.update({"metrics.json": json.dumps(metrics, ensure_ascii=False, indent=2) + "\n",
                  ...
                  "literal_wording_comparison.md": literal_comparison_table(metrics["literal_wording_comparison"]),
...
def self_test():
    ...
    assert list(aliases) == [f"C{i:02d}" for i in range(1, 9)] and len({tuple(v.values()) for v in aliases.values()}) == 8
    ...
    import tempfile
    with tempfile.TemporaryDirectory(prefix="fictional-analysis-test-") as folder:
        root = Path(folder)
        result, mapping, captured = build(root)
        assert result["totals"]["streams"] == 8 and result["totals"]["completed_hops"] == 0
        assert not result["all_arms_finalized"] and result["totals"]["usage_plus_reserves_usd"] == 0
        assert result["totals"]["budget_limit_usd"] == 10
        assert "n=1" in result["literal_wording_comparison"]["scope"]
        assert "incomplete snapshot" in literal_comparison_table(result["literal_wording_comparison"])
        save(root, result, mapping, captured)
        assert len(list((root / "analysis/masked_samples").glob("C*.md"))) == 8
        assert len(list((root / "analysis/transcripts").glob("*/*.md"))) == 8
        saved = json.loads((root / "analysis/metrics.json").read_text(encoding="utf-8"))
        assert saved["totals"] == result["totals"]
        row = comparison_row(result["streams"]["previous/gpt-6-luna"])
        assert row["completed_hops"] == 0 and row["final_completed_extracted_characters"] is None
